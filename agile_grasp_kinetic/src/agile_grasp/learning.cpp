	...
	const std::string& svm_filename, const Eigen::Matrix3Xd& cam_pos, bool is_plotting)
{
	...
	cv::Mat support_vector = svm->getSupportVectors();
	if(support_vector.cols == 0)
	{
		std::cout << " Exception:\n";
		return antipodal_hands;
	}
	//try
	//{
		//FIXME svm.load(svm_filename.c_str());
	//}
	...
	std::cout << " # of support vectors: " << support_vector.cols << "\n";
...
//		std::cout << "# descriptors = " << descriptors.size() << std::endl;
  	...
  	// train the SVM
	//FIXME CvSVMParams params;
	Ptr<ml::SVM> svm = ml::SVM::create();

	//FIXME params.svm_type = CvSVM::C_SVC;
	svm->setType(ml::SVM::C_SVC);

	if (uses_linear_kernel)
	{ 
		//FIXME params.kernel_type = CvSVM::LINEAR; 
		svm->setKernel(ml::SVM::LINEAR);
	}
	else
	{
		//FIXME params.kernel_type = CvSVM::POLY;
		svm->setKernel(ml::SVM::POLY);
		//TODO params.degree = 2;
		svm->setDegree(2);
	}
	//FIXMECvSVM svm;


	//FIXME svm.train(features, labels, cv::Mat(), cv::Mat(), params);
